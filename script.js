...
async function _generateImage() {
  // Image uses /api/image on server - key check is server-side
...
function handleVideoRefUpload(type, event) {
  const file = event.target && event.target.files ? event.target.files[0] : null;
  if (!file) return;
  if (!file.type.startsWith('image')) { showToast('Please select an image file', 'error'); return; }
  if (file.size > 10 * 1024 * 1024) { showToast('Image too large (max 10 MB)', 'error'); return; }
  const previewEl = document.getElementById(type === 'start' ? 'videoStartPreview' : 'videoEndPreview');
  const reader = new FileReader();
  reader.onload = function(ev) {
    const dataUrl = ev.target.result;
    videoRefs[type] = { base64: dataUrl.split(',')[1], mime: file.type || 'image/jpeg', dataUrl: dataUrl };
    if (previewEl) {
      previewEl.innerHTML = '<img src="' + dataUrl + '" alt="' + type + ' frame" '
        + 'style="max-width:100%;max-height:80px;border-radius:8px;object-fit:cover;display:block;margin-top:4px;">';
    }
    if (type === 'start') {
      const eb = document.getElementById('videoEndRefBlock');
      if (eb) eb.classList.remove('video-ref-disabled');
    }
    showToast((type === 'start' ? 'Start' : 'End') + ' frame set! ✓', 'success');
  };
...
async function generateVideo() {
  ...
  const remaining = getRemainingVideoQuota();
  if (remaining <= 0) {
    showUpgradeModal(userPlan === 'free' ? 'pro' : 'proplus',
      'You reached your daily video limit. Upgrade to continue.');
    return;
  }
  ...
  const aspectChip = document.querySelector('#videoAspectGroup .chip.active');
  const aspectVal  = (aspectChip && aspectChip.dataset.value) ? aspectChip.dataset.value : '16:9';

  // TWO-PATH LOGIC
  // Path A: reference image uploaded -> include base64 in body
  // Path B: no reference image       -> prompt-only
  var requestBody;
  if (videoRefs.start && videoRefs.start.base64) {
    requestBody = {
      prompt: prompt, duration: videoDuration, plan: userPlan, aspect: aspectVal,
      startImageBase64: videoRefs.start.base64,
      startImageMime:   videoRefs.start.mime || 'image/jpeg'
    };
    if (videoRefs.end && videoRefs.end.base64) {
      requestBody.endImageBase64 = videoRefs.end.base64;
      requestBody.endImageMime   = videoRefs.end.mime || 'image/jpeg';
    }
  } else {
    requestBody = { prompt: prompt, duration: videoDuration, plan: userPlan, aspect: aspectVal };
  }
  ...
  try {
    ...
    if (player && data.videoUrl) {
      ...
      player.load();
      // Fetch as Blob for iOS/Android playback and download
      try {
        var vHeaders = authToken ? { Authorization: 'Bearer ' + authToken } : {};
        const vRes    = await fetch(data.videoUrl, { headers: vHeaders });
        if (!vRes.ok) throw new Error('blob fetch failed');
        ...
        player.play().catch(function(){});
        if (dlBtn) {
          dlBtn.href = data.videoUrl; dlBtn.download = 'jeethy-video.mp4';
    ...
    if (statusEl) statusEl.innerHTML =
      '<i class="fas fa-circle-check"></i> ' + (data.message || 'Video ready') + ' · Preview below';
    if (resultEl) {
      resultEl.style.display = 'block';
      setTimeout(function(){ resultEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 150);
    }
