...
function updateVideoUI() {
  ...
  [startInput, endInput].forEach((input) => {
    if (!input) return;
    // Never hard-disable - allow click always, let label CSS handle visual state
    input.disabled = false;
    input.removeAttribute("disabled");
    const lbl = input.closest('label');
    if (lbl) {
      lbl.style.opacity = refsAllowed ? "1" : ".45";
      lbl.style.pointerEvents = refsAllowed ? "auto" : "none";
      lbl.style.cursor = refsAllowed ? "pointer" : "not-allowed";
    }
  });
...
showUpgradeModal();
  ...
  // Step 1: Submit job (fast response < 5s, avoids Cloudflare 100s timeout)
  let jobId = null;
  try {
    ...
    if (!data.jobId) throw new Error(data.error || "No job ID returned from server");
    jobId = data.jobId;
  } catch (err) {
  ...
  // Step 2: Show processing UI immediately
  ...
  const dlEl     = document.getElementById("videoDownloadBtn");
  ...
  const pollTimer = setInterval(async () => {
    ...
    if (pollCount > maxPolls) {
      ...
      if (statusEl) statusEl.innerHTML = `<i class="fas fa-circle-exclamation"></i> Timed out. Please try again.`;
    ...
    try {
      ...
      if (data.status === "error") {
        clearInterval(pollTimer);
        btn.disabled = false; btn.innerHTML = oldHtml;
        if (statusEl) statusEl.innerHTML = `<i class="fas fa-circle-xmark"></i> Error: ${data.error}`;
        showToast(data.error || "Video failed", "error");
        return;
      }
      ...
      if (data.status === "done" && data.videoUrl) {
        ...
        // Update quota
        if (Number.isFinite(data.usageCount)) setVideoUsageToday(data.usageCount);
        else setVideoUsageToday(getVideoUsageToday() + 1);
        updateVideoUI();
        ...
        if (playerEl) {
          ...
          // Use direct src (best for Android WebView + mobile browsers)
          playerEl.src = absUrl;
          playerEl.type = "video/mp4";
          // Fallback link
          ...
          playerEl.onerror = () => {
            if (statusEl) statusEl.innerHTML = `<i class="fas fa-circle-check"></i> Video ready! Â· <a href="${absUrl}" target="_blank" style="color:#22d3ee;text-decoration:underline;font-weight:700;">Open Video â†—</a>`;
            const shell = document.querySelector(".video-player-shell");
            if (shell) {
              shell.innerHTML = `<div style="padding:28px 16px;text-align:center;background:rgba(34,211,238,.06);border-radius:12px;border:1px solid rgba(34,211,238,.2);">
        ...
        if (dlEl) {
          dlEl.onclick = async (e) => {
            ...
            try {
              ...
              const resp = await fetch(dlUrl, { headers: authToken ? { Authorization: `Bearer ${authToken}` } : {} });
        ...
        if (statusEl) statusEl.innerHTML = `<i class="fas fa-circle-check"></i> Video generated successfully. Â· Preview below`;
        if (playerEl) {
      ...
      // Still processing
      if (statusEl) {
        const elapsed = pollCount * 8;
        statusEl.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Generating... ${elapsed}s`;
      }
    } catch (netErr) {
      console.warn("[video poll] network error:", netErr.message);
      // Continue polling on network blip
    }
  }, 8000);
}
